...
print_summary() {
    ...
    printf "      ./home_linux_mac.sh %s\n" "$home_args"
    printf "    ${D}  It will ask: Quick start (no sudo) or Full system setup (sudo).${X}\n"
    printf "    ${D}  Or pass --sudo / --no-sudo to skip the prompt.${X}\n\n"
    printf "    ${Y}Windows:${X}\n"
    local win_args="--token $tunnel_tok"
    [ -n "$ssh_ca_key" ] && win_args="${win_args} --ca-key \"$ssh_ca_key\""
    [ -n "$SSH_HOST" ] && win_args="${win_args} --ssh-host $SSH_HOST"
    printf "      home_windows.bat %s\n" "$win_args"
    printf "    ${D}  It will ask: Quick start (no admin) or Full system setup (admin).${X}\n"
    printf "    ${D}  Or pass --admin / --no-admin to skip the prompt.${X}\n\n"
