...
def print_summary(subdomain, emails, tsnet_ok=False):
    ...
    print(f"""
  Your endpoints:
    Browser SSH : {C}https://{ssh_host}{X}  (CF Access login)
...
""")
    ...
    # Build the home installer command with arguments
    home_args = f"--token {tunnel_tok}"
    if ssh_ca_key:
        home_args += f' --ca-key "{ssh_ca_key}"'
    home_args += f" --ssh-host {ssh_host}"

    print(f"""  {G}{B}What to do next:{X}

  {C}STEP 1 -- Set up your HOME machine{X} (the one you SSH into)
  Copy installers/ to your home machine, then run:

    {Y}Linux / Mac:{X}
      chmod +x home_linux_mac.sh
      sudo ./home_linux_mac.sh {home_args}

    {Y}Windows (as Administrator):{X}
      home_windows.bat {tunnel_tok} "{ssh_ca_key}" {ssh_host}

  {C}STEP 2 -- Set up your WORK machine{X} (the one you connect from)
  Copy installers/ to your work machine, then run:

    {Y}Linux / Mac:{X}
      chmod +x work_linux_mac.sh && ./work_linux_mac.sh --ssh-host {ssh_host}

    {Y}Windows (no admin needed):{X}
      work_windows.bat {ssh_host}

  Windows .bat files work even when GPO blocks PowerShell.

  {C}STEP 3 -- Connect{X}
    Browser : https://{ssh_host}  (email OTP login)
    CLI     : ssh YOUR_USER@{ssh_host}
""")
